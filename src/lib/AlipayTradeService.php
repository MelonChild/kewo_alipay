...
use Kewo\Alipay\lib\aop\AopClient;
class AlipayTradeService {
	...
	function __construct($alipay_config){
		...
		if(empty($this->appid)||trim($this->appid)==""){
			throw new AliPayException("appid should not be NULL!",1002);
		}
		if(empty($this->private_key)||trim($this->private_key)==""){
			throw new AliPayException("private_key should not be NULL!",1002);
		}
		if(empty($this->alipay_public_key)||trim($this->alipay_public_key)==""){
			throw new AliPayException("alipay_public_key should not be NULL!",1002);
		}
		if(empty($this->charset)||trim($this->charset)==""){
			throw new AliPayException("charset should not be NULL!",1002);
		}
		if(empty($this->gateway_url)||trim($this->gateway_url)==""){
			throw new AliPayException("gateway_url should not be NULL!",1002);
		}
 	...
 	*/
	function pagePay($builder,$return_url,$notify_url,$mobile=false) {
	
		$biz_content=$builder->getBizContent();
		//打印业务参数
		$this->writeLog($biz_content);
	
		$request = new aop\request\AlipayTradePagePayRequest();
	
		if($mobile) {
			$request->setApiMethodName();
		}
